...
const PriceSlabModal = (props) => {
    ...
    const onSubmit = async (e) => {
        ...
        const id = 11;
        await addNewPriceSlab(id, priceSlabForm).then((res) => {
            console.log(res);
        }).catch((err) => {
            console.error(err);
        });
        ...
        <Modal size='xl' isOpen={props.isOpen}>
            ...
            <ModalContent className="price-slab-modal-content">
                ...
                <ModalBody >
                    ...
                    <form className="grid grid-cols-1 gap-3 md:grid-cols-2" encType="Multipart/form-data" onSubmit={onSubmit}>
                        <FormControl size='sm' isRequired>
                            <FormLabel className='form-label-sm'>MOQ</FormLabel>
                            <NumberInput size='sm' onChange={(e) => {handleNumberInputChange(e, 'moq')}}>
                        ...
                        <FormControl size='sm' isRequired>
                            ...
                            <NumberInput size='sm' onChange={(e) => {handleNumberInputChange(e, 'price')}}>
